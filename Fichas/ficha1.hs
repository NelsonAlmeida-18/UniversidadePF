...
vertices(Rectangulo (Cartesiano a b) (Cartesiano c d)) = [Cartesiano a b , Cartesiano c b, Cartesiano a d, Cartesiano c d]
vertices(Rectangulo (Cartesiano a b) (Polar c d))= do
    let z = c
    let x = z * (cos(d*(pi/180)))
    let y = z * (sin(d*(pi/180)))
    [Cartesiano a b, Cartesiano x b, Cartesiano a d, Cartesiano x y]
vertices(Rectangulo (Polar a b) (Polar c d))= do
    let z = a
    let x = z * (cos(b*(pi/180)))
    let y = z * (sin(b*(pi/180)))
    let p = c
    let q = p * (cos(d*(pi/180)))
    let r = p * (sin(d*(pi/180)))
    [Cartesiano x y, Cartesiano q y, Cartesiano x r, Cartesiano q r]

--fazer em polar todos
area :: Figura -> Double
area (Triangulo (Cartesiano a b) (Cartesiano c d) (Cartesiano e f)) = 
    let g = dist' (Cartesiano a b)(Cartesiano c d)
        h = dist' (Cartesiano c d)( Cartesiano e f)
        i = dist' (Cartesiano e f) (Cartesiano a b)
        j = (g +h +i) / 2 -- semi perimetro
    in sqrt ((j*(j-g)*(j-h)*(j-i)))

area (Rectangulo (Cartesiano a b) (Cartesiano c d))= (c-a)*(d-b)
area (Circulo (Cartesiano a b) r) = pi*(r^2) 

--d) fazer
perimetro' :: Figura -> Double
perimetro' (Triangulo (Cartesiano a b) (Cartesiano c d) (Cartesiano e f)) = 
    let g = dist' (Cartesiano a b)(Cartesiano c d)
        h = dist' (Cartesiano c d)( Cartesiano e f)
        i = dist' (Cartesiano e f) (Cartesiano a b)
    in abs(g)+abs(h)+abs(i)
--fazer para polar mas código muito repetitivo

perimetro' (Circulo (Cartesiano a b)r) = 2*pi*r
perimetro' (Circulo (Polar a b)r) = 2*pi*r
perimetro' (Rectangulo (Cartesiano a b) (Cartesiano c d))=2*(abs(a-c))+2*(abs(d-b))
--fazer polar   
